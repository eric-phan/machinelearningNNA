...
def optimized_route(truck, current_location, sorted_truck, sorted_truck_idx):
    ...
    # Find the nearest package to current location
    next_package = None
    ...
    if next_package:
        # Add the nearest package to sorted truck
        sorted_truck.append(next_package)
        sorted_truck_idx.append(int(next_package.location))

        # Remove the selected package from the truck, which simulates delivery
